...
//  InsangNeCut
//
//  Created by Hamlit Jason on 2022/12/08.
//
...
class IAPTestViewController: UIViewController, IAPTraits {
  ...
  private let productID = "homeboothInnerTest"
    ...
    Self.iapService.getPaymentStateObservable()
      ...
      .subscribe(onNext: { ss, state in
        ...
        case .purchased, .restored:
            print("✅ canUseRandomColor true")
          ss.canUseRandomColor = true
        default:
            print("✅✅ canUseRandomColor false")
  ...
  @objc private func didTapBuyButton() {
    Self.iapService.purchase(productID: self.productID)
      .withUnretained(self)
      .subscribe(onError: { print("🤩 didTapBuy error \($0)") })
      .disposed(by: self.disposeBag)
  }
}
